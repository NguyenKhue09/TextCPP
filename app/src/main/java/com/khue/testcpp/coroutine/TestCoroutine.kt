...
class TestCoroutine {
    ...
    val y = MutableSharedFlow<Int>(replay = 1, extraBufferCapacity = 1)
    ...
    private suspend fun runFakeCalData(i: Int): Int = withTimeout(2000) {
        ...
        if (Random.nextInt(0, 1001).also { Log.d("TestCoroutine", "random num $it") }
                .mod(2) == 0) delay(3000) // fake call data
        else delay(1000)
        getI().also {
